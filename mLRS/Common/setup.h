...
#error Unknown Frequencyband !
#endif
#elif defined FREQUENCY_BAND_915_MHZ_FCC && defined FREQUENCY_BAND_868_MHZ && defined FREQUENCY_BAND_433_MHZ && defined FREQUENCY_BAND_70_CM_HAM
...
void setup_default_bindphrase(char* const s, uint8_t config_id, const char* bindphrase_default) {
    strcpy(s, bindphrase_default);  // 先复制默认值 "mlrs.0"
    if (config_id == 0)
        return;
...
// 将指定配置ID的所有参数设置为默认值
// config_id: 配置ID (0-9)，用于支持多个独立的配置组
void setup_default(uint8_t config_id) {
    //--------- 通用配置 (发射端和接收端共用) ---------
    char bind_phrase[6 + 1];
    setup_default_bindphrase(bind_phrase, config_id, BIND_PHRASE);  // 根据config_id生成默认配对短语，如 "mlrs.0", "mlrs.1" 等
    strcpy(Setup.Common[config_id].BindPhrase, bind_phrase);

    Setup.Common[config_id].FrequencyBand = SETUP_RF_BAND;  // 射频频段 (如 868MHz, 915MHz, 2.4GHz)
    Setup.Common[config_id].Mode = SETUP_MODE;              // 工作模式 (如 19Hz, 31Hz, 50Hz 帧率)
    Setup.Common[config_id].Ortho = SETUP_RF_ORTHO;         // 正交模式 (用于提高抗干扰能力)

    //--------- 发射端配置 ---------
    Setup.Tx[config_id].Power = SETUP_TX_POWER;                           // 发射功率
    Setup.Tx[config_id].Diversity = SETUP_TX_DIVERSITY;                   // 分集模式 (单天线/双天线)
    Setup.Tx[config_id].SerialDestination = SETUP_TX_SERIAL_DESTINATION;  // 串口数据输出目标
    Setup.Tx[config_id].ChannelsSource = SETUP_TX_CHANNELS_SOURCE;        // 通道输入源 (CRSF/INPort/mBridge/None)
    Setup.Tx[config_id].ChannelOrder = SETUP_TX_CHANNEL_ORDER;            // 通道顺序 (AETR/TAER等)
    Setup.Tx[config_id].InMode = SETUP_TX_IN_MODE;                        // 输入模式 (SBUS/CRSF等)
    Setup.Tx[config_id].SerialBaudrate = SETUP_TX_SERIAL_BAUDRATE;        // 串口波特率
    Setup.Tx[config_id].SendRadioStatus = SETUP_TX_SEND_RADIO_STATUS;     // 是否发送无线电状态
    Setup.Tx[config_id].Buzzer = SETUP_TX_BUZZER;                         // 蜂鸣器配置
    Setup.Tx[config_id].MavlinkComponent = SETUP_TX_MAV_COMPONENT;        // Mavlink组件设置
    Setup.Tx[config_id].PowerSwitchChannel = POWER_SWITCH_CHANNEL_OFF;    // 电源开关通道

    // WiFi 桥接配置 (仅ESP32/ESP8266)
    Setup.Tx[config_id].WifiProtocol = WIFI_PROTOCOL_UDP;  // WiFi协议 (UDP/TCP)
    Setup.Tx[config_id].WifiChannel = WIFI_CHANNEL_6;      // WiFi通道
    Setup.Tx[config_id].WifiPower = WIFI_POWER_MED;        // WiFi功率

    //--------- 接收端配置 ---------
    Setup.Rx.Power = SETUP_RX_POWER;                          // 接收功率
    Setup.Rx.Diversity = SETUP_RX_DIVERSITY;                  // 分集模式
    Setup.Rx.ChannelOrder = SETUP_RX_CHANNEL_ORDER;           // 通道顺序
    Setup.Rx.OutMode = SETUP_RX_OUT_MODE;                     // 输出模式 (SBUS/CRSR等)
    Setup.Rx.OutRssiChannelMode = SETUP_RX_OUT_RSSI_CHANNEL;  // RSSI输出通道配置
    Setup.Rx.FailsafeMode = SETUP_RX_FAILSAFE_MODE;           // 失控保护模式
    Setup.Rx.SerialPort = SETUP_RX_SERIAL_PORT;               // 串口选择
    Setup.Rx.SerialBaudrate = SETUP_RX_SERIAL_BAUDRATE;       // 串口波特率
    Setup.Rx.SerialLinkMode = SETUP_RX_SERIAL_LINK_MODE;      // 串口链路模式 (透明/MSP等)
    Setup.Rx.SendRadioStatus = SETUP_RX_SEND_RADIO_STATUS;    // 是否发送无线电状态
    Setup.Rx.SendRcChannels = SETUP_RX_SEND_RC_CHANNELS;      // 是否发送RC通道
    Setup.Rx.PowerSwitchChannel = POWER_SWITCH_CHANNEL_OFF;   // 电源开关通道

    // 失控保护输出通道默认值
    for (uint8_t ch = 0; ch < 12; ch++) {
        Setup.Rx.FailsafeOutChannelValues_Ch1_Ch12[ch] = 0;  // 通道1-12 默认为中位 (0)
    }
    for (uint8_t ch = 0; ch < 4; ch++) {
        Setup.Rx.FailsafeOutChannelValues_Ch13_Ch16[ch] = 1;  // 通道13-16 默认为低 (1)
    }
}
...
// DUALBAND 868/915 MHz & 433 MHz
    ...
    if ((Setup.Tx[config_id].ChannelsSource == CHANNEL_SOURCE_CRSF) && (Setup.Tx[config_id].SerialDestination == SERIAL_DESTINATION_MBRDIGE)) {
    ...
    for (uint8_t n = 0; n < sizeof(Setup.Rx.spare) / sizeof(Setup.Rx.spare[0]); n++)
...
void configure_mode(uint8_t mode) {
    ...
    switch (Config.Mode) {
        ...
        case MODE_FLRC_111HZ:
            ...
            Config.frame_rate_hz = 111, Config.Sx.LoraConfigIndex = 0;
...
void setup_configure_config(uint8_t config_id) {
    ...
    switch (Config.FrequencyBand) {
        case SETUP_FREQUENCY_BAND_2P4_GHZ:
            Config.Sx.FrequencyBand = SX_FHSS_CONFIG_FREQUENCY_BAND_2P4_GHZ;
            break;
        case SETUP_FREQUENCY_BAND_915_MHZ_FCC:
        ...
        case SETUP_FREQUENCY_BAND_433_MHZ:
            Config.Sx.FrequencyBand = SX_FHSS_CONFIG_FREQUENCY_BAND_433_MHZ;
            break;
        case SETUP_FREQUENCY_BAND_70_CM_HAM:
            Config.Sx.FrequencyBand = SX_FHSS_CONFIG_FREQUENCY_BAND_70_CM_HAM;
            break;
        case SETUP_FREQUENCY_BAND_866_MHZ_IN:
            Config.Sx.FrequencyBand = SX_FHSS_CONFIG_FREQUENCY_BAND_866_MHZ_IN;
            break;
    }
    ...
    if ((Setup.Tx[config_id].ChannelsSource == CHANNEL_SOURCE_MBRIDGE) || (Setup.Tx[config_id].SerialDestination == SERIAL_DESTINATION_MBRDIGE)) {
...
void setup_init(void) {
    ...
    if (ee_status == EE_STATUS_OK) {
        ee_status = setup_retrieve_from_EEPROM();  // 3. 从EEPROM读取配置
    }
    if (ee_status != EE_STATUS_OK) {  // try it a 2nd time// 4. 如果失败，重试一次
        setup_clear();
        ee_status = ee_init();
        if (ee_status == EE_STATUS_OK) {
    ...
    if (ee_status != EE_STATUS_OK)
        setup_clear();  // force default // 5. 仍失败，强制使用默认配置

    // 验证配置有效性（1022-1028行）
    if ((strncmp(Setup.MarkerStr, SETUP_MARKER_NEW_STR, 16) != 0) && (strncmp(Setup.MarkerStr, SETUP_MARKER_OLD_STR, 16) != 0)) {
